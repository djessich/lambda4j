...
public interface ThrowableToLongFunction<T, X extends Throwable> extends Lambda, ToLongFunction<T> {
    ...
    static <T, X extends Throwable> ThrowableToLongFunction<T, X> of(
            @Nonnull final ThrowableToLongFunction<T, X> expression) {
        return expression;
    }
     ...
     * @implSpec Overrides the {@link ToLongFunction#applyAsLong(Object)} method by using a redefinition as default
     ...
     *
     * @return A composed {@code ToLongFunction2} that applies this function to its input and nests the thrown {@code
