...
public interface ThrowableObjLongConsumer<T> extends ObjLongConsumer<T> {
     ...
     * ThrowableObjLongConsumer} is added as suppressed to the given one, unless it is an unchecked exception.
     * <p>
     ...
     * @throws NullPointerException If the given argument is {@code null}
     */
    default <X extends Exception> ThrowableObjLongConsumer<T> orThrowAlways(Class<X> clazz) {
        Objects.requireNonNull(clazz);
        return (t, value) -> {
            try {
                acceptThrows(t, value);
