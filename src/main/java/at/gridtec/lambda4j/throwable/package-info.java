...
/**
 * This package allows us to use lambdas which are able to throw any {@link java.lang.Exception}s. Therefore we are able
 * to call any methods from lambdas without remembering to catch exceptions. The thrown {@link java.lang.Exception} is
 * always sneakily thrown from any throwable lambda, unless its a {@link java.lang.RuntimeException}. This means that
 * there is no need to catch the thrown exception, nor to declare that you throw it using the <em>throws</em> keyword.
 * The exception is still thrown, but the Java compiler stops warning about it.
 * <p>
 * However, when using this throwable lambda, be aware of the following consequences: <ol> <li>If the calling code is to
 ...
 * more information on the default methods available, visit the documentation of the appropriate throwable lambda.
 *
