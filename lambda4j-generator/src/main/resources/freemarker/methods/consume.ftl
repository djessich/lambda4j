...
<#if !LambdaUtils.isOfTypeConsumer(lambda)>
    ...
    <#assign inputLambda = LambdaUtils.searchByInputOneType(LambdaUtils.getConsumerType(), 1, lambda.returnType, lambda.throwable)>
    <#-- get output consumer lambda using a search for all the input types of global lambda; throwable flag will also be handled -->
    <#assign outputLambda = LambdaUtils.searchByInputTypes(LambdaUtils.getConsumerType(), lambda.arity, lambda.inputOneType!"", lambda.inputTwoType!"", lambda.inputThreeType!"", lambda.throwable)>
 ...
 */
${annotation.nonnull}
default ${outputLambda.name}${types.buildGenericParameterTypeString(outputLambda)} consume(${annotation.nonnull} final ${inputLambda.name}${types.buildGenericParameterTypeString(inputLambda)} ${inputLambda.type.simpleName}) {
