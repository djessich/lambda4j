...
public interface ThrowableToIntTriFunction<T, U, V, X extends Throwable> extends Lambda {
    ...
    static <T, U, V, X extends Throwable> int call(
            @Nonnull ThrowableToIntTriFunction<? super T, ? super U, ? super V, ? extends X> function, T t, U u,
            V v) throws X {
    ...
    static <T, U, V, X extends Throwable> ThrowableToIntTriFunction<T, U, V, X> onlyThird(
            @Nonnull ThrowableToIntFunction<? super V, ? extends X> function) {
        Objects.requireNonNull(function);
     ...
     */
    @Nonnull
    default ThrowableTriConsumer<T, U, V, X> consume(@Nonnull ThrowableIntConsumer<? extends X> consumer) {
     ...
     * {@code ThrowableToIntTriFunction} with methods provided by the {@code JDK}.
     *
     ...
     *
     * @return A composed {@link ToIntTriFunction} that applies this function to its input and nests the thrown {@code
     * Throwable} from it.
     ...
     * @return A composed {@link ToIntTriFunction} that first applies this function to its input, and then applies the
     ...
     */
    @Nonnull
    default ToIntTriFunction<T, U, V> recover(
            @Nonnull Function<? super Throwable, ? extends ToIntTriFunction<? super T, ? super U, ? super V>> recover) {
        Objects.requireNonNull(recover);
        return (t, u, v) -> {
            try {
                return applyAsIntThrows(t, u, v);
            } catch (Error e) {
                throw e;
            } catch (Throwable throwable) {
                ToIntTriFunction<? super T, ? super U, ? super V> function = recover.apply(throwable);
