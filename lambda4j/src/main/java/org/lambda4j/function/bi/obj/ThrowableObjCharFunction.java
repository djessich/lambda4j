 ...
 * result which is able to throw any {@link Throwable}. This is a (reference, char) specialization of {@link
 * ThrowableBiFunction}.
 * <p>
...
public interface ThrowableObjCharFunction<T, R, X extends Throwable> extends Lambda {
    ...
    static <T, R, X extends Throwable> ThrowableObjCharFunction<T, R, X> of(
            @Nullable ThrowableObjCharFunction<T, R, X> expression) {
        return expression;
    }
    ...
    static <T, R, X extends Throwable> ThrowableObjCharFunction<T, Optional<R>, X> lift(
            @Nonnull ThrowableObjCharFunction<? super T, ? extends R, ? extends X> partial) {
    ...
    static <T, R, X extends Throwable> R call(
            @Nonnull ThrowableObjCharFunction<? super T, ? extends R, ? extends X> function, T t,
            char value) throws X {
     ...
     */
    @Nonnull
    default ThrowableCharFunction<R, X> applyThrowsPartially(T t) {
     ...
     */
    @Nonnull
    default ThrowableFunction<T, R, X> applyThrowsPartially(char value) {
     ...
     */
    @Nonnull
    default ThrowableObjCharConsumer<T, X> consume(@Nonnull ThrowableConsumer<? super R, ? extends X> consumer) {
    ...
    default ThrowableObjCharFunction<T, R, X> memoized() {
        if (isMemoized()) {
            ...
            Map<Pair<T, Character>, R> cache = new ConcurrentHashMap<>();
     ...
     */
    @Nonnull
    default ObjCharFunction<T, R> nest(@Nonnull Function<? super Throwable, ? extends RuntimeException> mapper) {
    ...
    default ObjCharFunction<T, R> recover(
            @Nonnull Function<? super Throwable, ? extends ObjCharFunction<? super T, ? extends R>> recover) {
        ...
        return (t, value) -> {
            try {
                ...
                ObjCharFunction<? super T, ? extends R> function = recover.apply(throwable);
    ...
    /**
     * Returns a composed {@link ObjCharFunction} that applies this function to its input and sneakily throws the thrown
