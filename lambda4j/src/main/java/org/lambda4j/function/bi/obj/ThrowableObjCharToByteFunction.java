 ...
 * {@code byte}-valued result which is able to throw any {@link Throwable}. This is a (reference, char) specialization
...
public interface ThrowableObjCharToByteFunction<T, X extends Throwable> extends Lambda {
    ...
    static <T, X extends Throwable> ThrowableObjCharToByteFunction<T, X> of(
            @Nullable ThrowableObjCharToByteFunction<T, X> expression) {
        return expression;
    }
    ...
    static <T, X extends Throwable> byte call(
            @Nonnull ThrowableObjCharToByteFunction<? super T, ? extends X> function, T t, char value) throws X {
     ...
     */
    @Nonnull
    default ThrowableObjCharPredicate<T, X> andThenToBoolean(@Nonnull ThrowableBytePredicate<? extends X> after) {
        Objects.requireNonNull(after);
     ...
     */
    @Nonnull
    default ThrowableObjCharConsumer<T, X> consume(@Nonnull ThrowableByteConsumer<? extends X> consumer) {
    ...
    default ThrowableObjCharToByteFunction<T, X> memoized() {
        if (isMemoized()) {
            ...
            Map<Pair<T, Character>, Byte> cache = new ConcurrentHashMap<>();
    ...
    default ObjCharToByteFunction<T> recover(
            @Nonnull Function<? super Throwable, ? extends ObjCharToByteFunction<? super T>> recover) {
        ...
        return (t, value) -> {
            try {
                ...
                ObjCharToByteFunction<? super T> function = recover.apply(throwable);
