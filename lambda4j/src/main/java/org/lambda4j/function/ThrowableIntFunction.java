...
/**
 * Represents an operation that accepts one {@code int}-valued input argument and produces a result which is able to
...
@FunctionalInterface
public interface ThrowableIntFunction<R, X extends Throwable> extends Lambda, IntFunction<R> {

    /**
     * Constructs a {@link ThrowableIntFunction} based on a lambda expression or a method reference. Thereby the given
    ...
    static <R, X extends Throwable> ThrowableIntFunction<R, X> of(
            @Nullable ThrowableIntFunction<R, X> expression) {
        return expression;
    }

    /**
     * Lifts a partial {@link ThrowableIntFunction} into a total {@link ThrowableIntFunction} that returns an {@link
     * Optional} result.
     *
     * @param <R> The type of return value from the function
     * @param <X> The type of the throwable to be thrown by this function
     * @param partial A function that is only defined for some values in its domain
     * @return A partial {@code ThrowableIntFunction} lifted into a total {@code ThrowableIntFunction} that returns an
     * {@code Optional} result.
     * @throws NullPointerException If given argument is {@code null}
     */
    @Nonnull
    static <R, X extends Throwable> ThrowableIntFunction<Optional<R>, X> lift(
            @Nonnull ThrowableIntFunction<? extends R, ? extends X> partial) {
        Objects.requireNonNull(partial);
        return value -> Optional.ofNullable(partial.applyThrows(value));
    }

    /**
     * Calls the given {@link ThrowableIntFunction} with the given argument and returns its result.
     *
     * @param <R> The type of return value from the function
     * @param <X> The type of the throwable to be thrown by this function
     * @param function The function to be called
     * @param value The argument to the function
     * @return The result from the given {@code ThrowableIntFunction}.
     * @throws NullPointerException If given argument is {@code null}
     * @throws X Any throwable from this functions action
     */
    static <R, X extends Throwable> R call(@Nonnull ThrowableIntFunction<? extends R, ? extends X> function,
            int value) throws X {
     ...
     */
    @Nonnull
    static <R, X extends Throwable> ThrowableIntFunction<R, X> constant(R ret) {
        return value -> ret;
    }
     ...
     * @param value The argument to the function
     * @return The return value from the function, which is its result.
     * @apiNote This method mainly exists to use this {@link ThrowableIntFunction} in JRE specific methods only
     * accepting {@link IntFunction}. If this function should be applied, then the {@link #applyThrows(int)} method
     * should be used.
     * @apiNote Overrides the {@link IntFunction#apply(int)} method by using a redefinition as default method. This
     * implementation calls the {@link #applyThrows(int)} method of this function and catches the eventually thrown
    ...
    default <A> ThrowableFunction<A, R, X> compose(
        ...
        return a -> applyThrows(before.applyAsIntThrows(a));
    }
    ...
    default ThrowableBooleanFunction<R, X> composeFromBoolean(
        ...
        Objects.requireNonNull(before);
        return value -> applyThrows(before.applyAsIntThrows(value));
    }
     ...
     */
    @Nonnull
    default ThrowableByteFunction<R, X> composeFromByte(@Nonnull ThrowableByteToIntFunction<? extends X> before) {
        Objects.requireNonNull(before);
        return value -> applyThrows(before.applyAsIntThrows(value));
    }
     ...
     */
    @Nonnull
    default ThrowableCharFunction<R, X> composeFromChar(@Nonnull ThrowableCharToIntFunction<? extends X> before) {
        Objects.requireNonNull(before);
        return value -> applyThrows(before.applyAsIntThrows(value));
    }
    ...
    /**
     * Returns a composed {@link ThrowableIntFunction} that first applies the {@code before} operator to its input, and
     * then applies this function to the result. This method is just convenience, to provide the ability to execute an
     * operation which accepts {@code int} input, before this primitive function is executed.
     *
     * @param before The operator to apply before this function is applied
     * @return A composed {@code ThrowableIntFunction} that first applies the {@code before} operator to its input, and
     * then applies this function to the result.
     * @throws NullPointerException If given argument is {@code null}
     * @implSpec The input argument of this method is able to handle primitive values. In this case this is {@code
     * int}.
     */
    @Nonnull
    default ThrowableIntFunction<R, X> composeFromInt(@Nonnull ThrowableIntUnaryOperator<? extends X> before) {
        Objects.requireNonNull(before);
        return value -> applyThrows(before.applyAsIntThrows(value));
    }
     ...
     */
    @Nonnull
    default ThrowableLongFunction<R, X> composeFromLong(@Nonnull ThrowableLongToIntFunction<? extends X> before) {
        Objects.requireNonNull(before);
        return value -> applyThrows(before.applyAsIntThrows(value));
    }
     ...
     */
    @Nonnull
    default ThrowableIntPredicate<X> andThenToBoolean(@Nonnull ThrowablePredicate<? super R, ? extends X> after) {
        Objects.requireNonNull(after);
     ...
     */
    @Nonnull
    default ThrowableIntUnaryOperator<X> andThenToInt(@Nonnull ThrowableToIntFunction<? super R, ? extends X> after) {
        Objects.requireNonNull(after);
     ...
     */
    @Nonnull
    default ThrowableIntConsumer<X> consume(@Nonnull ThrowableConsumer<? super R, ? extends X> consumer) {
     ...
     */
    @Nonnull
    default ThrowableIntFunction<R, X> reversed() {
        return this;
    }
     ...
     * forever.
     *
     * @return A memoized (caching) version of this {@code ThrowableIntFunction}.
    ...
    default ThrowableIntFunction<R, X> memoized() {
        if (isMemoized()) {
            ...
            return (ThrowableIntFunction<R, X> & Memoized) value -> {
     ...
     * ThrowableIntFunction} with methods provided by the {@code JDK}.
     *
     * @return A composed {@code ThrowableFunction} which represents this {@code ThrowableIntFunction}.
     */
    @Nonnull
    default ThrowableFunction<Integer, R, X> boxed() {
        return this::applyThrows;
    }

    /**
     * Returns a composed {@link IntFunction2} that applies this function to its input and nests the thrown {@link
     * Throwable} from it. The {@code Throwable} is nested (wrapped) in a {@link ThrownByFunctionalInterfaceException},
     * which is constructed from the thrown {@code Throwable}s message and the thrown {@code Throwable} itself.
     *
     * @return A composed {@link IntFunction2} that applies this function to its input and nests the thrown {@code
     * Throwable} from it.
     ...
     * @return A composed {@link IntFunction2} that first applies this function to its input, and then applies the
     ...
     */
    @Nonnull
    default IntFunction2<R> recover(
            @Nonnull Function<? super Throwable, ? extends IntFunction<? extends R>> recover) {
        ...
        return value -> {
            try {
                ...
                IntFunction<? extends R> function = recover.apply(throwable);
    ...
    /**
     * Returns a composed {@link IntFunction2} that applies this function to its input and sneakily throws the thrown
     ...
     *
     * @return A composed {@link IntFunction2} that applies this function to its input and sneakily throws the thrown
     * {@link Throwable} from it, unless it is of type {@link RuntimeException} or {@link Error}.
     * @implNote If thrown {@link Throwable} is of type {@link RuntimeException} or {@link Error}, it is thrown as-is
     * and thus not sneakily thrown.
     */
    @Nonnull
    default IntFunction2<R> sneakyThrow() {
        return value -> {
            try {
                return applyThrows(value);
