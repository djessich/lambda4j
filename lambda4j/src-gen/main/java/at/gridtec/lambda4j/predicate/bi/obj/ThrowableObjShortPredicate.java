...
public interface ThrowableObjShortPredicate<T, X extends Throwable> extends Lambda {
    ...
    static <T, X extends Throwable> ThrowableObjShortPredicate<T, X> of(
            @Nullable final ThrowableObjShortPredicate<T, X> expression) {
        return expression;
    }
    ...
    default ObjShortPredicate<T> recover(
            @Nonnull final Function<? super Throwable, ? extends ObjShortPredicate<? super T>> recover) {
        Objects.requireNonNull(recover);
        return (t, value) -> {
            try {
                return this.testThrows(t, value);
            } catch (Throwable throwable) {
                final ObjShortPredicate<? super T> predicate = recover.apply(throwable);
