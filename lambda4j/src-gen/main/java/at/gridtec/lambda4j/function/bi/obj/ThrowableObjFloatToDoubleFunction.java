...
public interface ThrowableObjFloatToDoubleFunction<T, X extends Throwable> extends Lambda {
    ...
    static <T, X extends Throwable> ThrowableObjFloatToDoubleFunction<T, X> of(
            @Nullable final ThrowableObjFloatToDoubleFunction<T, X> expression) {
        return expression;
    }
    ...
    default ObjFloatToDoubleFunction<T> recover(
            @Nonnull final Function<? super Throwable, ? extends ObjFloatToDoubleFunction<? super T>> recover) {
        Objects.requireNonNull(recover);
        return (t, value) -> {
            try {
                return this.applyAsDoubleThrows(t, value);
            } catch (Throwable throwable) {
                final ObjFloatToDoubleFunction<? super T> function = recover.apply(throwable);
