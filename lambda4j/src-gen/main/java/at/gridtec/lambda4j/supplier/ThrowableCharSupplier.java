...
/**
 * Represents a supplier of {@code char}-valued results which is able to throw any {@link Throwable}.
 * This is a primitive specialization of {@link ThrowableSupplier}.
 * <p>
 * There is no requirement that a distinct result be returned each time the supplier is invoked.
 * <p>
 * This is a {@link FunctionalInterface} whose functional method is {@link #getAsCharThrows()}.
 *
...
@FunctionalInterface
public interface ThrowableCharSupplier<X extends Throwable> extends Lambda {

    /**
     * Constructs a {@link ThrowableCharSupplier} based on a lambda expression or a method reference. Thereby the given
     ...
     * @return A {@code ThrowableCharSupplier} from given lambda expression or method reference.
     ...
     */
    static <X extends Throwable> ThrowableCharSupplier<X> of(@Nullable final ThrowableCharSupplier<X> expression) {
        return expression;
    }

    /**
     * Calls the given {@link ThrowableCharSupplier} with the given argument and returns its result.
     *
     * @param <X> The type of the throwable to be thrown by this supplier
     * @param supplier The supplier to be called
     * @return The result from the given {@code ThrowableCharSupplier}.
     * @throws NullPointerException If given argument is {@code null}
     * @throws X Any throwable from this suppliers action
     */
    static <X extends Throwable> char call(@Nonnull final ThrowableCharSupplier<? extends X> supplier) throws X {
        Objects.requireNonNull(supplier);
        return supplier.getAsCharThrows();
    }
     ...
     */
    @Nonnull
    static <X extends Throwable> ThrowableCharSupplier<X> constant(char ret) {
        return () -> ret;
    }
     ...
     */
    @Nonnull
    default <S> ThrowableSupplier<S, X> andThen(@Nonnull final ThrowableCharFunction<? extends S, ? extends X> after) {
        Objects.requireNonNull(after);
        return () -> after.applyThrows(getAsCharThrows());
    }
     ...
     */
    @Nonnull
    default ThrowableBooleanSupplier<X> andThenToBoolean(@Nonnull final ThrowableCharPredicate<? extends X> after) {
        Objects.requireNonNull(after);
        return () -> after.testThrows(getAsCharThrows());
    }
     ...
     */
    @Nonnull
    default ThrowableByteSupplier<X> andThenToByte(@Nonnull final ThrowableCharToByteFunction<? extends X> after) {
        Objects.requireNonNull(after);
        return () -> after.applyAsByteThrows(getAsCharThrows());
    }
     ...
     */
    @Nonnull
    default ThrowableCharSupplier<X> andThenToChar(@Nonnull final ThrowableCharUnaryOperator<? extends X> after) {
        Objects.requireNonNull(after);
        return () -> after.applyAsCharThrows(getAsCharThrows());
    }
    ...
    default ThrowableDoubleSupplier<X> andThenToDouble(
        ...
        Objects.requireNonNull(after);
        return () -> after.applyAsDoubleThrows(getAsCharThrows());
    }
     ...
     */
    @Nonnull
    default ThrowableFloatSupplier<X> andThenToFloat(@Nonnull final ThrowableCharToFloatFunction<? extends X> after) {
        Objects.requireNonNull(after);
        return () -> after.applyAsFloatThrows(getAsCharThrows());
    }
     ...
     */
    @Nonnull
    default ThrowableIntSupplier<X> andThenToInt(@Nonnull final ThrowableCharToIntFunction<? extends X> after) {
        Objects.requireNonNull(after);
        return () -> after.applyAsIntThrows(getAsCharThrows());
    }
     ...
     */
    @Nonnull
    default ThrowableLongSupplier<X> andThenToLong(@Nonnull final ThrowableCharToLongFunction<? extends X> after) {
        Objects.requireNonNull(after);
        return () -> after.applyAsLongThrows(getAsCharThrows());
    }
     ...
     */
    @Nonnull
    default ThrowableShortSupplier<X> andThenToShort(@Nonnull final ThrowableCharToShortFunction<? extends X> after) {
        Objects.requireNonNull(after);
        return () -> after.applyAsShortThrows(getAsCharThrows());
    }
     ...
     * @throws NullPointerException If given argument is {@code null}
     * @implNote Due to the fact that a {@link ThrowableCharSupplier} receives no input, we do not need to pass an
     ...
     */
    @Nonnull
    default ThrowableConsumer<Void, X> consume(@Nonnull final ThrowableCharConsumer<? extends X> consumer) {
        Objects.requireNonNull(consumer);
        return ignored -> consumer.acceptThrows(getAsCharThrows());
    }

    /**
     * Returns a memoized (caching) version of this {@link ThrowableCharSupplier}. Whenever it is called, the return
     ...
     * forever.
     *
     * @return A memoized (caching) version of this {@code ThrowableCharSupplier}.
     ...
     */
    @Nonnull
    default ThrowableCharSupplier<X> memoized() {
        if (isMemoized()) {
            return this;
        } else {
            AtomicReference<Character> cache = new AtomicReference<>();
            return (ThrowableCharSupplier<X> & Memoized) () -> {
                Character returnValue = cache.get();
                if (returnValue == null) {
                    synchronized (cache) {
                        returnValue = cache.get();
                        if (returnValue == null) {
                            returnValue = this.getAsCharThrows();
    ...
    /**
     * Returns a composed {@link ThrowableSupplier} which represents this {@link ThrowableCharSupplier}. Thereby the
     * primitive input argument for this supplier is autoboxed. This method provides the possibility to use this {@code
     * ThrowableCharSupplier} with methods provided by the {@code JDK}.
     *
     * @return A composed {@code ThrowableSupplier} which represents this {@code ThrowableCharSupplier}.
     */
    @Nonnull
    default ThrowableSupplier<Character, X> boxed() {
        return this::getAsCharThrows;
    }

    /**
     * Returns a composed {@link CharSupplier} that applies this supplier to its input and nests the thrown {@link
     * Throwable} from it. The {@code Throwable} is nested (wrapped) in a {@link ThrownByFunctionalInterfaceException},
     * which is constructed from the thrown {@code Throwable}s message and the thrown {@code Throwable} itself.
     *
     * @return A composed {@link CharSupplier} that applies this supplier to its input and nests the thrown {@code
     * Throwable} from it.
     ...
     */
    @Nonnull
    default CharSupplier nestWith(@Nonnull final Function<? super Throwable, ? extends RuntimeException> mapper) {
    ...
    /**
     * Returns a composed {@link CharSupplier} that applies this supplier to its input and sneakily throws the
     ...
     *
     * @return A composed {@link CharSupplier} that applies this supplier to its input and sneakily throws the thrown
     * {@link Throwable} from it, unless it is of type {@link RuntimeException} or {@link Error}.
     * @implNote If thrown {@link Throwable} is of type {@link RuntimeException} or {@link Error}, it is thrown as-is
     * and thus not sneakily thrown.
     */
    @Nonnull
    default CharSupplier sneakyThrow() {
        return () -> {
            try {
                return this.getAsCharThrows();
     ...
     * @return A composed {@link CharSupplier} that first applies this supplier to its input, and then applies the
     ...
     */
    @Nonnull
    default CharSupplier recover(@Nonnull final Function<? super Throwable, ? extends CharSupplier> recover) {
        ...
        return () -> {
            try {
                ...
                final CharSupplier supplier = recover.apply(throwable);
